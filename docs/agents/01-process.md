...
- Run `pnpm typecheck` and `pnpm test:run` to verify changes before finishing
...
```bash
swarm new "Implement auth redirect fix"     # create sandbox
swarm list                                  # see what's running
swarm open auth-redirect-fix                # resume a sandbox
swarm aider                                 # launch agent in current worktree
```
